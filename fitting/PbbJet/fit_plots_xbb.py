...
def fitb_plots(signal_name, jet_type, region, nrho, npt, pseudodata=False, decidata=False, logy=False):
	...
	# Untar plots dir
	cwd = os.getcwd()
	os.chdir(config.get_datacard_directory(signal_name, jet_type, qcd=pseudodata, decidata=decidata, region=region))
	os.system("tar -xzvf " + config.get_datacard_directory(signal_name, jet_type, qcd=pseudodata, decidata=decidata, region=region) + "/plots_combine_{}.tar.gz".format(fit_jobname))
	os.chdir(cwd)

	fit_file = TFile(config.get_datacard_directory(signal_name, jet_type, qcd=pseudodata, decidata=decidata, region=region) + "/plots_combine_{}/mlfitcombine_{}.root".format(fit_jobname, fit_jobname), "READ")
	for cat in xrange(1, 7):
		for what in ["pass", "fail"]:
			...
			for background in background_list:
				#print "[debug] Background {} has {} bins".format(background, background_hists[background].GetNbinsX())
...
if __name__ == "__main__":
	...
	if args.poly_degrees == "":
		...
		poly_degrees_strings["CA15"] = "2:1,3:1,4:1,5:1,6:1,4:2,5:2"
	else:
		...
		for jet_type in args.jet_types.split(","):
			for region in args.regions.split(","):
				for poly_degree in poly_degree_pairs[jet_type]:
					fitb_plots(signal, jet_type, region, poly_degree[0], poly_degree[1], pseudodata=args.pseudodata, decidata=args.decidata, logy=False)

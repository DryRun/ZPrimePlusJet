...
def writeDataCard(boxes,workspace_path,sigs,bkgs,histoDict,options,datacard_path):
    obsRate = {}
    for box in boxes:
    ...
    for proc in sigs+bkgs:
        for box in boxes:
            ...
            if proc=='wqq' or proc=='zqq' or 'hqq' in proc or 'Sbb' in proc:
    ...
    # now nuisances
    datacard+=lumiString+veffString+bbeffString+znormEWString+znormQString+wznormEWString+mutriggerString+muidString+muisoString
    if do_jesjer:
        datacard += jesString+jerString
...
def main(options, args):
    ...
    boxes = ['pass', 'fail']
    sigs = []
    for model in ["Sbb"]: # PS, Zp
        for mass in [50, 75, 100, 125, 150, 200, 300]:
            sigs.append("{}{}".format(model, mass))
    ...
    for sig in sigs:
        ...
        writeDataCard(boxes,workspace_path,[sig],bkgs,histoDict,options,datacard_path)
        print '\ndatacard:\n'

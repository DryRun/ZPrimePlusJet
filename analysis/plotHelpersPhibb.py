...
def makeCanvasComparison(hs,legname,color,style,name,pdir="plots",lumi=30,ofile=None,unitnorm=False):
    ...
    if 'Phibb' in iname: 
        leg1.AddEntry(h,legname[iname],"l")
    else: 
...
def makeCanvasComparisonStackWData(hd,hs,hb,legname,color,style,outname,pdir="plots",lumi=30,ofile=None,normalize=False,ratio=True):
    ...
    if ratio:   
        ...
        print "[makeCanvasComparisonStackWData] KS score:"
        print ksScore
        print "[makeCanvasComparisonStackWData] Chi2 score:"
        print chiScore
        ratio.SetStats(0)
...
def makeCanvasRatio(h_denom,h_numer,legname,color,style,outname,pdir="plots",lumi=30,ofile=None,pt=None,f2params=None, minM=40, maxM=201, nr=2, np=1):
    ...
    h_denom.GetXaxis().SetRangeUser(minM,maxM)

    if pt is not None and f2params is not None:
        f1params = array.array('d',list(f2params))
        #f1params.append(pt)
        npar = len(f2params)
        f2 = ROOT.TF2("f2", polyn_msd_pt(nr, np),h_denom.GetXaxis().GetXmin(),h_denom.GetXaxis().GetXmax(),h_denom.GetYaxis().GetXmin(),h_denom.GetYaxis().GetXmax(),npar)
        f2.SetParameters(f2params)
        f1 = ROOT.TF1("f1",polyn_msd(nr, np, pt),h_denom.GetXaxis().GetXmin(),h_denom.GetXaxis().GetXmax(),npar)
    ...
    print "[makeCanvasRatio] KS score:"
    print ksScore
    print "[makeCanvasRatio] Chi2 score:"
    print chiScore
    ratio.SetStats(0)
    ratio.GetYaxis().SetRangeUser(0.,2.5)   
    ...
    if pt is not None:
        print "[debug] f1.Eval(100) = " + str(f1.Eval(100.))
...
# Return polynomial in rho and pT, with inputs [msd, pT]
...
# Return polynomial in rho and pT, with inputs [rho, pT]
def polyn_rho_pt(nrho, npt):
    def fun_rho_pt(x, par):
        rho = x[0]
        pt = x[1]
        ret = 0.
        for i in xrange(nrho + 1):
...
# Returns slice of polynomial for fixed pT. x=[msd]
def polyn_msd(nrho, npt, pt):
    def fun_msd(x, par):
        if pt <= 0.:
            return 0.
        msd = x[0]
        rho = ROOT.TMath.Log(msd**2/pt**2)
        ret = 0.
        for i in xrange(nrho + 1):
            for j in xrange(npt + 1):
                par_index = i + nrho*j
...
def fun5_1(x, par):
    rho = ROOT.TMath.Log((x[0] * x[0]) / (par[36] * par[36]))
    poly0 = par[0] * (    1.0 +  par[1] * rho +  par[2] * rho * rho +  par[3] * rho * rho * rho +  par[4] * rho * rho * rho * rho +  par[5] * rho * rho * rho * rho * rho)
    poly1 = par[0] * ( par[6] +  par[7] * rho +  par[8] * rho * rho +  par[9] * rho * rho * rho + par[10] * rho * rho * rho * rho + par[11] * rho * rho * rho * rho * rho) * par[36]
    poly2 = par[0] * (par[12] + par[13] * rho + par[14] * rho * rho + par[15] * rho * rho * rho + par[16] * rho * rho * rho * rho + par[17] * rho * rho * rho * rho * rho) * par[36] * par[36]
    poly3 = par[0] * (par[18] + par[19] * rho + par[20] * rho * rho + par[21] * rho * rho * rho + par[22] * rho * rho * rho * rho + par[23] * rho * rho * rho * rho * rho) * par[36] * par[36] * par[36]
    poly4 = par[0] * (par[24] + par[25] * rho + par[26] * rho * rho + par[27] * rho * rho * rho + par[28] * rho * rho * rho * rho + par[29] * rho * rho * rho * rho * rho) * par[36] * par[36] * par[36] * par[36]
...
def makeCanvasRatio2D(h_denom, h_numer, legname, color, style, outname, pdir="plots", lumi=30, ofile=None, rho_lo=-6, rho_hi=-2.1, nr=2, np=1):
    ...
    for i in xrange(1, npar):
        f2params[i] = 0.
    f2 = ROOT.TF2("f2", polyn_msd_pt(nr, np), ratio.GetXaxis().GetXmin(),ratio.GetXaxis().GetXmax(),ratio.GetYaxis().GetXmin(),ratio.GetYaxis().GetXmax(),npar)
    f2.SetParameters(f2params)
    #for i in range(0, npar):
    #    if i > ((np+1)*nr+1): # assuming np = max(np, nr)
    #        f2.FixParameter(i,0)
    fr = ratio.Fit('f2','RNS')
    print "2D fit chi2 / NDF = {} / {} = {}".format(f2.GetChisquare(), f2.GetNDF(), f2.GetChisquare() / f2.GetNDF())
    print "\tp = {}".format(f2.GetProb())
    #f2.Draw("surf")
    ratio.Draw('surf1')
    ...
    print '[makeCanvasRatio2D] chi2 = ', fr.Chi2()
    ...
    f2rho = ROOT.TF2("f2",polyn_rho_pt(nr, np),rho_lo,rho_hi,ratio.GetYaxis().GetXmin(),ratio.GetYaxis().GetXmax(),npar)
